...
enum Command {
    Discover {
        devices: mpsc::Sender<Vec<BleDevice>>,
    },
    EventScan {
    ...
    runtime.spawn(async move {
        while let Some(msg) = rx.recv().await {
            match msg {
                ...
                Command::Discover { devices } => inner_discover(devices).await.unwrap(),
            }
        }
    });
    Ok(())
...
pub fn stream_discover(sink: StreamSink<Vec<BleDevice>>, filter: Vec<String>) -> Result<()> {
    send(Command::EventScan { sink, filter })
}

pub async fn discover() -> Result<Vec<BleDevice>> {
    let (tx, mut rx) = mpsc::channel(1);
    send(Command::Discover { devices: tx })?;
    let devices = rx.recv().await.unwrap();
    log(format!("discoverd {} devices", devices.len()));
    Ok(devices)
}

async fn inner_discover(tx: mpsc::Sender<Vec<BleDevice>>) -> Result<()> {
    log("Starting inner discover");
    let mut devices = vec![];
    ...
    let discovered = central.peripherals().await?;
